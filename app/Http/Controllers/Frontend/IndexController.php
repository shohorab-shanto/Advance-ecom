    ...
    public function index()
{
    ...
    $skip_product_0 = $skip_category_0 ? 
        Product::where('status', 1)->where('category_id', $skip_category_0->id)->orderBy('id', 'DESC')->get() 
        : collect(); // Return an empty collection if null

    $skip_category_1 = Category::skip(1)->first();
    $skip_product_1 = $skip_category_1 ? 
        Product::where('status', 1)->where('category_id', $skip_category_1->id)->orderBy('id', 'DESC')->get() 
        : collect(); // Return an empty collection if null

    $skip_brand_2 = Brand::skip(4)->first();
    $skip_brand_product_2 = $skip_brand_2 ? 
        Product::where('status', 1)->where('brand_id', $skip_brand_2->id)->orderBy('id', 'DESC')->get() 
        : collect(); // Return an empty collection if null

    return view('frontend.index', compact(
        'categories', 'sliders', 'products', 'featured',
        'hotdeals', 'special_offer', 'special_deals',
        'skip_category_0', 'skip_product_0',
        'skip_category_1', 'skip_product_1',
        'skip_brand_2', 'skip_brand_product_2'
    ));
}
